...
// alert(data);
  ...
  // и поместить в массив: номер вопроса,имя файла, правильный ответ
   var  arr=data.split(';');   // получаем массив 0120312120,r
								//    0120312121,q
 //  alert (arr[0]);
...
//	  m_vopros_txt[i]=telo_voprosa[0];
 ...
 //  peremeshat_vse_voprosi(zapis);    // перемешиваем вопросы
  ...
  let i=0;
  for ( i = 0; i < zapis; i++)   {
     var div_вопрос_html="<div id='vopros"+m_nomera_vopros[i]+"'>";
	 var p_номер_вопроса_сайт="<p class='vopros_test'>Вопрос "+m_nomer_po_poradku[i]+"</p>";
     var sposob=0;
     sposob=Number(m_sposob[i]);
	
  switch(sposob)  {
    case 0:                      // картинка
	 var сам_вопрос=global_papka+m_voprosi[i]+'.PNG';
	 var текст_вопрос="<p><span id='v_" + m_nomera_vopros[i]+"'></span><img src=" +сам_вопрос+"></p>";     // поместили картинку
     var варианты_ответа="";
//	 let title_otvet="Ответ:<input type='text' id='z_"+m_nomera_vopros[i]+"'></p></div>";
	 ...
	 case 3:  // картинка и текст
	  ...
	  let vrem_num=m_voprosi[i];
	 if (perv_kartinka==0) {
		
		var kartinka =global_papka+global_predmet+'/'+global_klass+'/'+global_tema+'/'+ vrem_num +'.PNG';
		 perv_kartinka=perv_kartinka+1;
	  }
	  
	 
	// 
	
	  var текст_вопрос="<p><span id='v_" + m_nomera_vopros[i]+"'></span><img src=" +kartinka+" align='right'/><p class='vopros_test'>" + m_vopros_txt[i].split(/\r?\n/).join('<br/>')+"</p></p>";     // поместили картинку
	  var варианты_ответа="";
	  var title_otvet='';
//	  alert (recField.задачи_ответы[i].count);  
		for (m=0; m < Number(m_skolko_otvetov[i]); m++){
//             alert(recField.задачи_ответы[i].title[m]);
			 title_otvet=title_otvet + recField.задачи_ответы[i].title[m] + ":<input type='text' id='z_" + m_nomera_vopros[i]+ m + "' class='fieldInput'>";
		 }
	 //var поле_ответ="<p class='otvet_test'>Ответ:<input type='text' id='z_"+m_nomera_vopros[i]+"'></p></div>";
	 var поле_ответ="<p class='otvet_test'>"+ title_otvet + "</p></div>";
	 break;
	case 2:
	    var текст_вопрос="<p><span id='v_" + m_nomera_vopros[i]+"'></span><p class='vopros_test'>" + m_vopros_txt[i].split(/\r?\n/).join('<br/>')+"</p></p>";     // поместили текст
        var варианты_ответа="";
...
//            alert(recField.задачи_ответы[i].title[m]);
	 ...
	 break;
	case 1:                    // вопрос текст
      var поле_ответ="<p class='otvet_test'>Ответ:<input type='text' id='z_"+m_nomera_vopros[i]+"0' class='fieldInput'></p></div>";
	  var текст_вопрос="<p><span id='v_" + m_nomera_vopros[i]+"'></span><p class='vopros_test'>" + m_vopros_txt[i].split(/\r?\n/).join('<br/>')+"</p></p>";     // поместили текст
      var варианты_ответа="";
	  let variants=m_vopros_variant_txt[i].split('|');
	  let dlina_variants=(variants.length);
	  for (k=0; k < dlina_variants; k++) {
      //"<p ><span id='v_otv'></span><p class='variant_vopros'>" + m_vopros_variant_txt[i]+"</p></p>";     // поместили текст	 
	    
		варианты_ответа=варианты_ответа + "<p ><span id='v_otv'></span><p class='variant_vopros'>" + variants[k]+"</p></p>";     // поместили текст
	  }
	  var title_otvet='';
	  for (m=0; m < Number(m_skolko_otvetov[i]); m++){
//             alert(recField.задачи_ответы[i].title[m]);
...
function slojn() {                  // тестируемый определяет сложность теста
	 if (smena==0) {
		smena=1;
		//alert("Меняем на 1");
